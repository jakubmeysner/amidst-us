                ...
                if (countDownSeconds == 0) {
                    ...
                    countDownTask = plugin.server.scheduler.runTaskTimer(plugin, Runnable {
                        ...
                        if (countDownSeconds == 0 || votes.size == game.players.count { !it.dead }) {
                                        ...
                                        player.killCooldownTask =
                                            plugin.server.scheduler.runTaskTimer(plugin, Runnable {
                                                if (player.killCooldownSecondsLeft == 0) {
                                                    ...
                                                    player.bukkit.inventory.setItem(
                                                        1,
                                                        Player.ImpostorSwordItemStack.clone().apply {
                                                    ...
                                                    player.killCooldownSecondsLeft =
                                                        player.killCooldownSecondsLeft!! - 1
                                                }
                                            }, 0, 20)
                                    }
                                }
                            ...
                            bossBar?.progress =
                                (game.votingTimeSeconds - countDownSeconds!!).toDouble() / game.votingTimeSeconds
                        }
                    }, 20, 20)
                } else {
                    bossBar?.setTitle("${BukkitChatColor.YELLOW}Discussion time: ${countDownSeconds}s left")
                    bossBar?.progress =
                        (game.discussionTimeSeconds - countDownSeconds!!).toDouble() / game.discussionTimeSeconds
                }
            }, 20, 20)
        }, 3 * 20)
    }
