          ...
          if (
            ...
            if (game.emergencyMeetings.count { it.calledBy == player } >= game.emergencyMeetingsLimit) {
              player.bukkit.spigot().sendMessage(
                *ComponentBuilder("You have already used all your emergency meetings!").color(ChatColor.RED).create()
              )
            } else {
              val dif = game.emergencyMeetings.lastOrNull()?.let { Duration.between(it.endedAt, Instant.now()) }

              if (dif == null || dif.seconds >= game.emergencyMeetingsCooldownSeconds) {
                game.emergencyMeetings.add(EmergencyMeeting(plugin, game, EmergencyMeeting.Source.CALLED, player))
              } else {
                player.bukkit.spigot().sendMessage(
                  *ComponentBuilder(
