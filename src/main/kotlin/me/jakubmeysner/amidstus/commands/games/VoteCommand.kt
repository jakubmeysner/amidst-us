    ...
    if (args.size > 1) {
      ...
      if (game == null) {
        ...
        if (player.dead) {
          ...
          sender.spigot().sendMessage(
            *ComponentBuilder("No emergency meeting is currently in voting phase!").color(ChatColor.RED).create()
          )
        } else {
          if (args.isEmpty()) {
            ...
            if (emergencyMeeting.votes.containsKey(player)) {
              ...
              if (votePlayer == null) {
                sender.spigot().sendMessage(
                  *ComponentBuilder("There is no player with given name!").color(ChatColor.RED).create()
                )
